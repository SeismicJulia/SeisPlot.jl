...
function SeisPlotCoordinates(headers;style="sxsygxgy",cmap="Greys",aspect="auto",pclip=98,vmin="NULL",vmax="NULL",title=" ",xlabel=" ",xunits=" ",ylabel=" ",yunits=" ",ox=0,dx=1,oy=0,dy=1,dpi=100,wbox=6,hbox=6,name="NULL",interpolation="none",titlesize=20,labelsize=15,ticksize=10,fignum="NULL")
	...
	if (fignum == "NULL")
		...
		fig = pl.figure(num=fignum,figsize=(wbox, hbox), dpi=dpi, facecolor="w", edgecolor="k")
	end
	if (style == "sxsygxgy")
	...
	elseif (style == "fold")
		...
		im = pl.imshow(fold,cmap=cmap,vmin=a,vmax=b,extent=[ox,ox + (size(fold,2)-1)*dx,oy + (size(fold,1)-1)*dy,oy],aspect=aspect)
		ax = pl.gca()
		ax[:invert_yaxis]();
		pl.colorbar()
	else
